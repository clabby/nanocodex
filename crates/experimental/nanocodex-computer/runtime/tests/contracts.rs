...
fn native_drag_preserves_button_and_modifiers_through_public_cua() {
    ...
    eval(
        &mut host,
        "await app.drag([20,30],[80,90],{mouseButton:'middle',modifiers:['shift','control']});",
    );
    let state = engine
        .borrow_mut()
        .execute("fixture.state", &json!({}))
        .unwrap();
    assert_eq!(
        state["actions"][0],
    ...
    let state = engine
        ...
        .execute("fixture.state", &json!({}))
        .unwrap();
    assert_eq!(state["actions"][1]["button"], 0);
    assert_eq!(state["actions"][1]["modifiers"], json!([]));
}

#[test]
fn native_drag_rejects_invalid_modifiers_without_dispatch() {
    ...
    for extra in [
        json!({"modifiers":["a"]}),
        json!({"modifiers":["ctrl","control"]}),
        json!({"modifiers":"shift"}),
        json!({"modifiers":[null]}),
        json!({"mouse_button":3}),
        json!({"mouse_button":"other"}),
    ] {
        let mut args = json!({"app":"fixture://native","from":[20,30],"to":[80,90]});
        args.as_object_mut()
            .unwrap()
            .extend(extra.as_object().unwrap().clone());
        assert!(engine.execute("drag", &args).is_err(), "{args}");
    }
    assert_eq!(
        engine.execute("fixture.state", &json!({})).unwrap()["actions"],
        json!([])
    );
}
