...
impl RealtimeWebrtcSession {
    ...
    pub fn start(abort: AbortRegistration) -> Result<StartedRealtimeWebrtcSession> {
        let package = crate::package::discover()
            .clone()
            .ok_or_else(|| anyhow::anyhow!("voice package unavailable"))?;
        let build_commit = crate::package::BUILD_COMMIT.to_owned();
...
fn package_has_runtime(package: &std::path::Path) -> bool {
    let voice = package.join("nanocodex-resources/voice");
    let (helper, runtime) = if cfg!(target_os = "macos") {
        ("bin/nanocodex-voice-host", "lib/libgstreamer-1.0.0.dylib")
    } else if cfg!(windows) {
        ("bin/nanocodex-voice-host.exe", "bin/gstreamer-1.0-0.dll")
    } else {
