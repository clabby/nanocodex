...
it("account-local viewer admission preserves a real upgraded socket and first ready message", async () => {
  ...
  const request = new Request("https://account.test/v1/account/hands/view", { headers: { authorization: "Bearer fixture", upgrade: "websocket", "x-nanocodex-access": token } });
  ...
  const response = await routeManaged(request, { NANOCODEX_ACCESS_SECRET: secret,
    ...
    NANOCODEX_HAND_BROKER: { getByName(owner: string) { expect(owner).toBe("owner"); return { fetch: async () => new Response(null, { status: 101, webSocket: pair[0] }) }; } } as unknown as DurableObjectNamespace,
  }, new URL(request.url));
