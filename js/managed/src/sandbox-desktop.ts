...
type State = SandboxDesktopScope & { credential?: string; expiresAt?: number; processId?: string; transport?: "webrtc" };
...
export class SandboxDesktop {
  ...
  private async start(): Promise<void> {
    ...
    // Replace legacy frame-only publishers when an existing sandbox resumes.
    if (running && state.transport !== "webrtc") {
    ...
    const args = ["/usr/local/bin/nanocodex-remote", "server-host", "--width", "1920", "--height", "1080", "--url", endpoint,
