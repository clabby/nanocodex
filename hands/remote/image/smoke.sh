...
if ! docker exec "$container" sh -ec '
  ...
  timeout 4s sh -c "sleep 30 | NANOCODEX_SCREEN_ENCODER_HELPER=1 waymote-streamd --frame-rate 5 --bitrate 1000 --ffmpeg /usr/local/bin/nanocodex-remote" > /tmp/hand-smoke.h264 || status=$?
