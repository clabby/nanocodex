...
SP<IPC::Socket1::SCommand> g_refresh_command;
#else
SP<SHyprCtlCommand> g_status_command;
SP<SHyprCtlCommand> g_refresh_command;
#endif
...
void refresh_server() {
    try { reconcile_server(); }
    catch (const std::exception& error) { static_cast<void>(stop_server()); g_reconcile_error = error.what(); }
}
...
SP<IPC::Socket1::SCommand> register_refresh_command(HANDLE handle) {
    return HyprlandAPI::registerHyprCtlCommand(handle, IPC::Socket1::SCommand{
        .name = "cua:refresh", .match = IPC::Socket1::COMMAND_MATCH_EXACT,
        .handler = [](const IPC::Socket1::SRequest& request) { refresh_server(); return status(request); },
    });
}
...
SP<SHyprCtlCommand> register_refresh_command(HANDLE handle) {
    return HyprlandAPI::registerHyprCtlCommand(handle, SHyprCtlCommand{
        .name = "cua:refresh", .exact = true,
        .fn = [](eHyprCtlOutputFormat format, std::string args) { refresh_server(); return status(format, args); },
    });
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    g_refresh_command = register_refresh_command(handle);
    if (!g_refresh_command) throw std::runtime_error("failed to register cua:refresh");
