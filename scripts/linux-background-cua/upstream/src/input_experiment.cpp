...
struct InputExperiment::Impl {
    ...
    void request(Client& c, const std::vector<std::string>& f) {
        ...
        if (command == "DRAG") {
            const auto error = parse_drag_options(f, kProduction && c.route == InputRoute::independent, drag_options);
            if (!error.empty()) { send(c, refusal(error)); return; }
        } else if (f.size() != count) { send(c, refusal("invalid_request")); return; }
        ...
        if (command == "KEY") {
            ...
            if (command == "CLICK") {
                ...
                if (!consume_grant(c, cap)) return;
                if (!pointer_enter(c, x, y)) { send(c, refusal("client_not_bound")); return; }
                if (drag_options.modifiers && !keyboard_enter(c)) {
                    revoke("client_not_bound", true); send(c, refusal("client_not_bound")); return;
                }
                // Independent key() maintains only this lane's XKB state. Keep
                // modifiers down until after button release, including motion.
                drag_options.send_modifiers(true, [this](auto code, bool pressed) { key(code, pressed); });
                if (trace) trace->mark("agent_drag_start", lane + 1);
                button(drag_options.button, true);
                drag.emplace(Drag{&c, x, y, x2, y2, Clock::now(), static_cast<unsigned>(duration), DragGeometry{c.revision}, drag_options});
    ...
    void step() {
        ...
        if (drag) {
            const auto d = *drag;
            if (d.options.modifiers && !std::ranges::any_of(keyboards, [&](const auto& k) {
                return !k->dead && k->wl->resource() && k->focus == d.client->surface.lock();
            })) { revoke("client_not_bound"); return; }
            ...
            if (d.client->route == InputRoute::primary_foreground) {
                ...
                button(d.options.button, false);
                d.options.send_modifiers(false, [this](auto code, bool pressed) { key(code, pressed); });
                drag.reset(); ++dispatches;
